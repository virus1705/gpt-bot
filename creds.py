...
def create_new_token():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.info("get new iam_token")
        else:
            logging.error(f"error get iam_token. status-cod: {response.status_code}")
    except Exception as e:
        logging.error(f"error get iam_token: {e}")
...
def get_creds():
    try:
        ...
        if expiration < datetime.now():
            logging.info("expiration date iam_token expired")
            create_new_token()
    except:
        create_new_token()
